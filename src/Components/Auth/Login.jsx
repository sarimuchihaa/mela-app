...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${CustomerBaseURL}/login`, { email, password });
      if (response.status === 200) {
        ...
        // const role = userId && 'customer';
  
        // Store encrypted userId in local storage
        localStorage.setItem('customerid', encryptedUserId);
  
        // Store userId in UserContext
    ...
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-blue-500 to-blue-700">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 p-8 pt-16">
          ...
          <h2 className="text-xl text-center text-gray-800 pt-1 mb-8">Login with Customer Account</h2>
          ...
          <p className="text-center mt-6 text-sm text-gray-600">
            Don't Have Account? <Link to="/signup" className="text-blue-500 font-semibold hover:underline">Register Now</Link>
          </p>
        </div>
      </div>
    </div>
